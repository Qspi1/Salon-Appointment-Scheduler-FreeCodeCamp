...
# Connects to the postgres and salon DB
PSQL="psql -X --username=freecodecamp --dbname=salon --tuples-only -c"

# introduction to the salon.sh
echo -e "\n~~~~~ MY SALON ~~~~~\n"
...
SERVICES() {
  ...
  echo "$LIST_SERVICES" | while read SERVICE_ID BAR SERVICE # Displaying services in format
  do
...
APPOINT() {
  ...
  # if name doesnt exist on table --> insert new customer
  if [[ -z $NAME ]]
  then
